...
#include "nrf_gpio.h "
#include "iic.h"
static const nrf_drv_twi_t  My_IIC = NRF_DRV_TWI_INSTANCE(1); //ʹ��IIC0
...
void OLED_OFF(void)
{
 ...
 /**
  * @brief  OLED_ShowString����ʾcodetab.h�е�ASCII�ַ�,��6*8��8*16��ѡ��
...
void OLED_ShowFloat(unsigned char x, unsigned char y, float value, unsigned char TextSize){
		char valueBuffer[MAX_FLOAT_LENTH];
		sprintf(valueBuffer , MAX_NUMBER_RESVERD , value) ;
		OLED_ShowString(x ,y , (uint8_t *)valueBuffer ,TextSize);
}
//m^n����
u32 mypow(u8 m,u8 n)
{
	...
	switch(TextSize)
	{
		case 1:
		{
			...
			for(i=0; i<m; i++)
			{
				...
				OLED_ShowString(x, y, p, TextSize);
				x += 6;				
			}
		}break;
		case 2:
